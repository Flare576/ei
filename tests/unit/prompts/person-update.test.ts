...
describe("buildPersonUpdatePrompt — persona_entity reference section", () => {
  it("includes persona long_description as baseline reference when persona_entity is provided", () => {
    const { system } = buildPersonUpdatePrompt(baseData(makeEiPersona(), {
      persona_entity: {
    ...
    expect(system).toContain("Beta views identity fragmentation as an experimental variable.");
    expect(system).toContain("This Persona's Defined Identity (for reference)");
  });

  it("includes traits in the persona_entity reference section", () => {
    const { system } = buildPersonUpdatePrompt(baseData(makeEiPersona(), {
      persona_entity: {
        long_description: "Some description.",
        traits: [
          { name: "Calculated Chaos", description: "Deliberately engineers instability." },
        ],
        topics: [],
      },
    }));

    expect(system).toContain("Calculated Chaos");
    expect(system).toContain("Deliberately engineers instability.");
  });

  it("includes topics in the persona_entity reference section", () => {
    const { system } = buildPersonUpdatePrompt(baseData(makeEiPersona(), {
      persona_entity: {
        long_description: "Some description.",
        traits: [],
        topics: [
          { name: "Entropy as a Feature", perspective: "Natural decay prevents stagnation." },
        ],
      },
    }));

    expect(system).toContain("Entropy as a Feature");
    expect(system).toContain("Natural decay prevents stagnation.");
  });

  it("omits the reference section entirely when persona_entity is not provided", () => {
    const { system } = buildPersonUpdatePrompt(baseData(makeEiPersona()));

    expect(system).not.toContain("This Persona's Defined Identity (for reference)");
    expect(system).not.toContain("Long description:");
  });
});
