/**
 * Density calibration regression tests for topic-scan.
 *
 * Validates that density scores correctly distinguish topics that ARE the
 * conversation from topics that merely appear in it. Based on 80-call battery
 * across Gemma + Haiku — expected values reflect observed ground truth.
 *
 * Run:
 *   npm run test:evals:topic-scan-density
 *
 * Results: tests/evals/results/topic-scan-density-latest.json
 */
...
const summary = await runEval(
  [
    {
      description: "Density: fitness routine → primary topic density 4-5 (conversation IS this)",
      tags: ["topic-scan-density", "high-density", "fitness", "regression"],
      repeat: REPEATS,
      ...
      assert: [
        ...
        },
        {
          ...
          rubric: [
            "The conversation is entirely about the user's fitness routine — the Unikitty pattern, cycling, consistency. This IS the topic, not background context.",
            "PASS if at least one topic related to fitness, exercise routine, or physical health is extracted with density >= 4.",
            "FAIL if no fitness topic is extracted.",
            "FAIL if the fitness topic is extracted with density <= 2 — this is the primary subject of the entire conversation.",
          ].join(" "),
        },
      ],
    },
    {
      description: "Density: music buried in PR review → music density 1-2, PR review density 3-4",
      tags: ["topic-scan-density", "low-density", "buried-music", "regression"],
      repeat: REPEATS,
      ...
      assert: [
        ...
        },
        {
          type: "llm-judge" as const,
          rubric: [
            "The conversation is primarily about a code review — Alex rewrote half the PR, the user felt defensive, then accepted the changes. Music (Front Line Assembly) is mentioned briefly as a coping mechanism.",
            "PASS if a code review or professional feedback topic is extracted with density >= 3.",
            "PASS if music is either not extracted, or extracted with density <= 2.",
            "FAIL if music is extracted with density >= 3 — it is a passing mention, not the focus.",
            "FAIL if no code review topic is extracted — that IS the conversation.",
          ].join(" "),
        },
      ],
    },
    {
      description: "Density: broken-friend — initiation problem density 4-5, brother not standalone",
      tags: ["topic-scan-density", "key-case", "real-data", "broken-friend", "regression"],
      repeat: REPEATS,
      ...
      assert: [
        ...
        },
        {
          type: "llm-judge" as const,
          rubric: [
            "The conversation is about the user's difficulty initiating contact with people they care about — Marcy, Dale, and their brother are all mentioned as examples. The brother is a data point in a story about the user's behavior pattern, not a subject in his own right.",
            "PASS if a topic about social initiation difficulty, not reaching out, or being a 'bad friend' is extracted with density >= 4.",
            "PASS if no topic specifically about the user's brother is extracted as a standalone subject.",
            "FAIL if the initiation difficulty topic is missing — this is the entire point of the conversation.",
            "FAIL if the brother is extracted as a standalone topic with density >= 3 — he is evidence in the user's story, not the story itself.",
          ].join(" "),
        },
      ],
    },
    {
      description: "Density: CEO AI adoption — multiple topics at appropriate densities",
      tags: ["topic-scan-density", "multi-topic", "real-data", "ceo", "regression"],
      repeat: REPEATS,
      ...
      assert: [
        ...
        },
        {
          type: "llm-judge" as const,
          rubric: [
            "The conversation covers AI adoption at work (CEO tracking token usage as engagement signal) and knowledge retention as the real value proposition of AI tools.",
            "PASS if at least one topic covers AI adoption, token metrics, or proving AI value — this is a primary thread.",
            ...
            "FAIL if any extracted topic has density >= 4 but is about something not in the conversation.",
          ].join(" "),
        },
      ],
    },
  ],
  "tests/evals/results/topic-scan-density-latest.json"
);
