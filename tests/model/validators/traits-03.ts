...
let idealResponse: TraitsResponse | null = null;

async function loadIdealResponse(): Promise<TraitsResponse> {
  if (idealResponse) return idealResponse;
  
  const idealPath = path.join(__dirname, 'traits-03-ideal.json');
...
function traitsMatch(item1: TraitItem, item2: TraitItem): boolean {
  const type1 = normalizeValue(item1.type_of_trait);
  const type2 = normalizeValue(item2.type_of_trait);
  const val1 = normalizeValue(item1.value_of_trait);
  const val2 = normalizeValue(item2.value_of_trait);
...
function validateItem(item: unknown, requireReason: boolean): string | null {
  ...
  if (typeof obj.type_of_trait !== 'string' || !obj.type_of_trait) {
    return 'Item missing required field: type_of_trait (string)';
  }
  
  if (typeof obj.value_of_trait !== 'string' || !obj.value_of_trait) {
    return 'Item missing required field: value_of_trait (string)';
  }
...
export async function validate(response: string): Promise<ValidationResult> {
  ...
  const typedResponse = parsed as TraitsResponse;
  ...
  for (const idealItem of allIdealItems) {
    const match = allResponseItems.find(respItem => traitsMatch(idealItem, respItem));
    
    if (match) {
      foundItems.push(`${idealItem.value_of_trait}`);
      if (normalizeValue(match.type_of_trait) === normalizeValue(idealItem.type_of_trait)) typeMatches++;
      if (match.confidence === idealItem.confidence) confidenceMatches++;
    } else {
      missedItems.push(`${idealItem.value_of_trait}`);
    }
  }
  
  const extraItems = allResponseItems
    .filter(respItem => !allIdealItems.some(idealItem => traitsMatch(idealItem, respItem)))
    .map(item => `${item.value_of_trait}`);
