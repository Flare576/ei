...
## Acceptance Criteria
- [x] LLM prompts do NOT include `persona_groups` field in schema shown to model
- [x] LLM prompts do NOT instruct model to manage `persona_groups`
- [x] New concepts created by persona in group "Work" get `persona_groups: ["Work"]`
- [x] New concepts created by persona with no group get `persona_groups: []`
- [x] Existing concept discussed by persona in group "Personal" adds "Personal" to `persona_groups`
- [x] Concept with `persona_groups: ["Work"]` keeps "Work" when discussed by "Personal" persona (accumulates groups)
- [x] `last_updated` set by code after LLM response (note: `last_updated_by` not in schema - using `last_updated` only)
- [x] `learned_by` set for new concepts (note: `added` not in schema - using `last_updated` instead)
- [x] Concepts removed by LLM are handled gracefully (filtered out)
- [x] Code reconciliation tested with various scenarios (new, updated, removed concepts)
