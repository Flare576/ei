...
export class MockLLMServerImpl implements MockLLMServer {
  ...
  setResponseForType(requestType: string, response: MockResponse): void {
    ...
    if (this.config.enableLogging) {
      console.log(`Mock LLM Server: setResponseForType("${requestType}") set, now has ${this.responseTypeOverrides.size} type overrides`);
    }
  }
  ...
  private async handleChatCompletions(req: Request, res: Response): Promise<void> {
    ...
    if (this.config.enableLogging) {
      console.log(`Mock LLM Server: detected requestType="${requestType}", typeOverrides has "${requestType}": ${this.responseTypeOverrides.has(requestType)}`);
    }
