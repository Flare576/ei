...
let idealResponse: TopicsResponse | null = null;

async function loadIdealResponse(): Promise<TopicsResponse> {
  if (idealResponse) return idealResponse;
  
  const idealPath = path.join(__dirname, 'topics-03-ideal.json');
...
function topicsMatch(item1: TopicItem, item2: TopicItem): boolean {
  const val1 = normalizeValue(item1.value_of_topic);
  const val2 = normalizeValue(item2.value_of_topic);
  
  if (val1.includes(val2) || val2.includes(val1)) {
    return true;
  }
  
  const words1 = new Set(val1.split(/\s+/));
  const words2 = new Set(val2.split(/\s+/));
...
function validateItem(item: unknown, requireReason: boolean): string | null {
  ...
  if (typeof obj.type_of_topic !== 'string' || !obj.type_of_topic) {
    return 'Item missing required field: type_of_topic (string)';
  }
  
  if (typeof obj.value_of_topic !== 'string' || !obj.value_of_topic) {
    return 'Item missing required field: value_of_topic (string)';
  }
...
export async function validate(response: string): Promise<ValidationResult> {
  ...
  const typedResponse = parsed as TopicsResponse;
  ...
  for (const idealItem of allIdealItems) {
    const match = allResponseItems.find(respItem => topicsMatch(idealItem, respItem));
    
    if (match) {
      foundItems.push(`${idealItem.value_of_topic}`);
      if (normalizeValue(match.type_of_topic) === normalizeValue(idealItem.type_of_topic)) typeMatches++;
      if (match.confidence === idealItem.confidence) confidenceMatches++;
    } else {
      missedItems.push(`${idealItem.value_of_topic}`);
    }
  }
  
  const extraItems = allResponseItems
    .filter(respItem => !allIdealItems.some(idealItem => topicsMatch(idealItem, respItem)))
    .map(item => `${item.value_of_topic}`);
